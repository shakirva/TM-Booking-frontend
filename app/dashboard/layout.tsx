  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFF]">
            ...
            {navItems.map((item) => {
              ...
              return (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`flex items-center gap-3 px-4 py-2 rounded-lg font-medium transition-colors ${isActive ? 'bg-blue-600 text-white' : 'text-gray-700 hover:bg-gray-100'
                      }`}
                  >
                    {item.icon}
                    {item.label}
      ...
      <div className="flex-1 flex flex-col min-w-0">
              ...
              {segments.map((seg, idx) => (
                <React.Fragment key={idx}>
                  ...
                  {idx === segments.length - 1 ? (
                    <span className="text-black font-medium capitalize truncate">{seg.replace(/-/g, ' ')}</span>
                  ) : (
                    <a href={`/dashboard/${segments.slice(0, idx + 1).join("/")}`} className="hover:underline capitalize flex-shrink-0">{seg.replace(/-/g, ' ')}</a>
                  )}
                </React.Fragment>
              ))}
            </nav>
        ...
        {/* Page Content */}
        <main className="flex-1 bg-[#F8FAFC] min-w-0 overflow-x-hidden">
          <div className="max-w-full xl:max-w-7xl mx-auto p-4 md:p-8">{children}</div>
        </main>
        {/* Mobile bottom nav */}
        <MobileBottomNav />
      </div>
